...
COPY docker/typescript/package.json docker/typescript/package-lock.json /opt/dlr-typescript/
RUN npm ci --prefix /opt/dlr-typescript --ignore-scripts --no-audit --no-fund \
    && ln -s /opt/dlr-typescript/node_modules/typescript/bin/tsc /usr/local/bin/tsc
...
ENV PATH="/usr/local/go/bin:/app/.venv/bin:/opt/java/openjdk/bin:/usr/share/maven/bin:$PATH"

...
const evidenceRoot = resolve(
  specDir,
  process.env.DLR_WAVE_D_OUTPUT_DIR ?? "../../../docs/ui/m5-10-wave-d",
);
...
const adapterOne = {
  ...
  description: "Wave D fixture with long Chinese and English copy for responsive Workbench checks.",
  language: "python",
  adapter_type: "task",
  run_mode: "manual",
  timeout_seconds: 300,
...
async function screenshot(page: Page, name: string): Promise<string> {
  ...
  return `docs/ui/m5-10-wave-d/browser/${name}`;
}
...
async function runAdminScenario(page: Page, locale: Locale, width: number, diagnostics: ReturnType<typeof diagnosticsFor>, unknownRequests: string[], assistRequests: unknown[]) {
  ...
  records.push({
    ...
    width,
    scenario: "admin",
    screenshot: `docs/ui/m5-10-wave-d/browser/${locale}-${width}-adapter-switch.png`,
...
async function runErrorScenario(page: Page, locale: Locale, width: number, diagnostics: ReturnType<typeof diagnosticsFor>, unknownRequests: string[], assistRequests: unknown[]) {
  ...
  records.push({
    ...
    width,
    scenario: "error",
    screenshot: `docs/ui/m5-10-wave-d/browser/${screenshotName}`,
    console_errors: diagnostics.consoleErrors,
    expected_console_errors: diagnostics.expectedConsoleErrors,
    page_errors: diagnostics.pageErrors,
    http_errors: [],
...
test.afterAll(() => {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      ...
      wave: "M5.10 Wave D",
      scope: "Workbench and AI Assistant display/integration only",
      antd: "5.29.3",

...
const CONVERSIONS = new Set<any>([undefined, null, "string", "integer", "number", "boolean", "datetime"]);
function validPointer(path: any): any {
    if (typeof path !== "string" || Buffer.byteLength(path, "utf8") > MAX_POINTER_BYTES)
        return false;
    if (path === "")
        return true;
    if (!path.startsWith("/"))
...
function convert(value: any, kind: any): any {
...
function matches(item: any, rule: any): any {
    let value: any;
    let exists = true;
    try {
        value = pointer(item, rule.pointer);
    }
    catch (error: any) {
        if (!(error instanceof Error) || error.message !== "missing_pointer")
            throw error;
        exists = false;
    }
    if ((rule.op ?? "equals") === "exists")
        return exists === (rule.value ?? true);
    if ((rule.op ?? "equals") === "equals")
        return exists && jsonEquals(value, rule.value);
    throw new Error("unsupported_filter");
}
function stableKey(value: any): any {
    if (value === null || value === undefined)
        return [1, ""];
    return [0, canonicalJson(value)];
}
function jsonEquals(left: any, right: any): any {
    ...
    if (Array.isArray(left) || Array.isArray(right)) {
        return Array.isArray(left) && Array.isArray(right) && left.length === right.length
            && left.every((value: any, index: any) => jsonEquals(value, right[index]));
    }
    if ((left && typeof left === "object") || (right && typeof right === "object")) {
        ...
        return leftKeys.length === rightKeys.length
            && leftKeys.every((key: any, index: any) => key === rightKeys[index]
...
function canonicalJson(value: any): any {
    ...
    if (typeof encoded !== "string")
        throw new Error("output_not_json");
    return encoded;
}
function boundedInteger(input: any, key: any, fallback: any, maximum: any): any {
...
export function handle(context: Context, input: any): any {
    ...
    if (!Array.isArray(input.records) || !Array.isArray(input.mappings))
        throw new Error("records_and_mappings_required");
    const maxRecords = boundedInteger(input, "max_records", 10000, 100000);
    const maxFields = boundedInteger(input, "max_fields", 200, 1000);
    const maxOutputBytes = boundedInteger(input, "max_output_bytes", 4194304, 16777216);
    if (input.mappings.length > maxFields)
        throw new Error("invalid_limits");
    const filters = input.filters ?? [];
    if (!Array.isArray(filters))
        throw new Error("invalid_filters");
    if (filters.length > maxFields || filters.some((rule: any) => !rule || typeof rule !== "object"
    ...
    for (const [ordinal, record] of input.records.slice(0, maxRecords).entries()) {
        if (!filters.every((rule: any) => rule && typeof rule === "object" && matches(record, rule)))
            continue;
        const output = Object.create(null);
        const fieldSizes = new Map<any, any>();
        let objectBytes = 2;
        ...
        for (const mapping of input.mappings) {
            ...
            if (!validPointer(mapping.pointer))
                throw new Error("invalid_json_pointer");
            let value: any;
            try {
                value = pointer(record, mapping.pointer);
            }
            catch (error: any) {
                if (!(error instanceof Error) || error.message !== "missing_pointer")
                    throw error;
            ...
            try {
                ...
                if (mapping.target === sortField)
                    sortValue = converted;
                if (mapping.target === dedupeField)
                    dedupeValue = converted;
                if (recordLimited)
                    continue;
                ...
                if (previousSize === undefined && fieldSizes.size > 0)
                    nextObjectBytes += 1;
    ...
    const bounded = candidates.flatMap((item: any) => item.output === null ? [] : [item.output]);

...
async function tc3(op: any, region: any, page: any, size: any, context: Context, timeout: any, maximum: any): Promise<any> { const access = context.secrets.get("TENCENTCLOUD_SECRET_ID"), secret = context.secrets.get("TENCENTCLOUD_SECRET_KEY"); if (!access || !secret)
    ...
    throw new Error("provider_api_error");
} const pageValue = await boundedJson(response, maximum); if (pageValue.payload.Response?.Error)
    throw new Error("provider_api_error"); return { payload: pageValue.payload.Response ?? pageValue.payload, bytes: pageValue.bytes }; }
...
function boundedResult(mode: any, input: any, assets: any, relationships: any, pages: any, failures: any, partial: any, limitReached: any, maxBytes: any): any {
    const keptAssets: any[] = [...assets];
    const keptRelationships: any[] = [...relationships];
    let keptFailures = failures.slice(0, 50);
    let boundedPartial = partial;
    let boundedLimit = limitReached;
    const build = () => {
    ...
    const size = (value: any) => Buffer.byteLength(JSON.stringify(value), "utf8");
    let result = build();
    ...
    if (!boundedPartial) {
        boundedPartial = true;
        boundedLimit = true;
        keptFailures = ["bounded"];
        result = build();
    }
    if (size(result) > maxBytes && keptFailures.some((item: any) => item && typeof item === "object")) {
        keptFailures = [...new Set<any>(keptFailures.map((item: any) => item && typeof item === "object" ? String(item.error ?? "bounded") : String(item)))];
        result = build();
    }
    while (size(result) > maxBytes && keptRelationships.length) {
        keptRelationships.pop();
        boundedPartial = true;
        boundedLimit = true;
        result = build();
    }
...
export async function handle(context: Context, input: any): Promise<any> {
    ...
    if (typeof input.account !== "string" || !input.account || input.account.length > 256
        ...
        || input.regions.some((value: any) => typeof value !== "string" || !value || value.length > 128)) {
            ...
            try {
                for (let pageNumber = 1;; pageNumber += 1) {
                    ...
                    if (input.fixture_pages) {
                        ...
                        const responseLimit = Math.min(4194304, maxBytes - sourceBytes);
                        if (responseLimit <= 0) {
                            partial = true;
                            limitReached = true;
                            break scan;
    ...
    if (mode === "preview") {
        return boundedResult(mode, input, assets, relationships, pages, failures, partial, limitReached, maxBytes);
    }
    if (partial || Date.now() >= deadline) {
        return boundedResult(mode, input, assets, relationships, pages, failures, true, limitReached, maxBytes);
    }
    const summary: Record<string, any> = { assets: assets.length, relationships: relationships.length, pages, failures: [] };

...
func cloudPage(ctx *Context, requestCtx context.Context, op []any, region string, page, size int, query map[string]*string, maximum int) (any, error) {
	access, secret := ctx.Secrets.Get("TENCENTCLOUD_SECRET_ID"), ctx.Secrets.Get("TENCENTCLOUD_SECRET_KEY")
	if access == "" || secret == "" {
		return nil, errors.New("missing_credential")
	}
	service, host, action, version := stringValue(op[1]), stringValue(op[2]), stringValue(op[3]), stringValue(op[4])
	body := encoded(object{"Offset": (page - 1) * size, "Limit": size})
	now := time.Now().UTC()
	timestamp := strconv.FormatInt(now.Unix(), 10)
	date := now.Format("2006-01-02")
	canonical := strings.Join([]string{"POST", "/", "", "content-type:application/json; charset=utf-8\nhost:" + strings.ToLower(host) + "\nx-tc-action:" + strings.ToLower(action) + "\n", "content-type;host;x-tc-action", hashText(string(body))}, "\n")
	scope := date + "/" + service + "/tc3_request"
	toSign := strings.Join([]string{"TC3-HMAC-SHA256", timestamp, scope, hashText(canonical)}, "\n")
	key := mac(mac(mac([]byte("TC3"+secret), date), service), "tc3_request")
	signature := fmt.Sprintf("%x", mac(key, toSign))
	headers := http.Header{"Content-Type": []string{"application/json; charset=utf-8"}, "X-TC-Action": []string{action}, "X-TC-Version": []string{version}, "X-TC-Timestamp": []string{timestamp}, "X-TC-Region": []string{region}, "Authorization": []string{"TC3-HMAC-SHA256 Credential=" + access + "/" + scope + ", SignedHeaders=content-type;host;x-tc-action, Signature=" + signature}}
	if token := ctx.Secrets.Get("TENCENTCLOUD_TOKEN"); token != "" {
		headers.Set("X-TC-Token", token)
	}
	u, _ := url.Parse("https://" + host + "/")
	data, status, _, e := httpRead(requestCtx, "POST", u, headers, body, maximum)
	if e != nil || status < 200 || status >= 300 {
		return nil, errors.New("provider_api_error")
	}
	var payload object
	if json.Unmarshal(data, &payload) != nil {
		return nil, errors.New("provider_api_error")
	}
	response := mapping(payload["Response"])
	if response == nil {
		return payload, nil
	}

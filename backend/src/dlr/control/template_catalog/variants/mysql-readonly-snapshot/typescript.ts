...
function normalizeMysqlCell(value: any, field: any): any {
...
function connectionOptions(dsn: any): any {
    try {
...
function connect(connection: any): any {
    return new Promise((resolve: any, reject: any) => connection.connect((error: any) => error ? reject(error) : resolve()));
}
function command(connection: any, sql: any, values: any = undefined): any {
...
function close(connection: any): any {
    return new Promise((resolve: any) => connection.end(() => resolve()));
}
export async function handle(context: Context, input: any): Promise<any> {
    ...
    try {
        ...
        query.once("fields", (value: any) => {
            try {
                names = columnNames(value);
                fields = value;
            }
            catch (error: any) {
                fieldsError = error;
        ...
        const consume = () => {
            for (const rawRow of pending) {
                ...
                let normalized: any;
                try {
                    normalized = rawRow.map((value: any, at: any) => normalizeMysqlCell(value, fields[at]));
                }
                catch {
                    partial = true;
                    abortConnection = true;

...
function positive(value: any, fallback: any, maximum: any): any { return Number.isInteger(value) && value > 0 ? Math.min(value, maximum) : fallback; }
function digest(value: any): any { return crypto.createHash("sha256").update(JSON.stringify(value)).digest("hex"); }
function headers(context: Context): any {
...
async function boundedBytes(response: any, maximum: any, tooLargeCode: any): Promise<any> {
...
async function getPage(context: Context, url: any, maximum: any, deadline: any): Promise<any> {
    for (let attempt = 0; attempt < 3; attempt += 1) {
        const remaining = deadline - Date.now();
        if (remaining <= 0)
            throw new Error("servicenow_timeout");
        ...
        if (response.status === 429 || response.status >= 500) {
            ...
            if (attempt === 2)
                throw new Error("servicenow_retry_limit");
        ...
        if (!Array.isArray(payload.result))
            throw new Error("servicenow_invalid_result");
        return [payload.result, bytes.length];
    }
    throw new Error("servicenow_request_failed");
}
function display(record: any, key: any): any { const value = record[key]; return value && typeof value === "object" && !Array.isArray(value) ? value.display_value : value; }
function keyComponent(value: any): any { return value.replaceAll("%", "%25").replaceAll(":", "%3A"); }
function asset(account: any, record: any): any {
...
function snapshotResult(assets: any, pages: any, failure: any, partial: any, offset: any, assetCount: any = assets.length): any {
...
function partialSyncResult(input: any, assets: any, pages: any, failure: any, offset: any): any {
    const summary: Record<string, any> = {
...
function candidateFits(assetCount: any, assetItemBytes: any, pages: any, offset: any, maxBytes: any): any {
...
async function post(base: any, path: any, body: any, token: any, idem: any, deadline: any): Promise<any> {
    const remaining = deadline - Date.now();
    if (remaining <= 0)
...
async function sync(context: Context, input: any, assets: any, summary: any, deadline: any): Promise<any> {
    const scan = input.scan_id, scope = input.source_scope;
    const valid = /^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,127}$/;
    if (typeof scan !== "string" || !valid.test(scan))
        throw new Error("invalid_scan_id");
    if (typeof scope !== "string" || !valid.test(scope))
        throw new Error("invalid_source_scope");
    // 此处读取凭据：请在本适配器的“凭据绑定”中配置与 get(...) 参数一致的绑定键。
    const base = input.cmdb_base_url, token = context.secrets.get("CMDB_TOKEN");
    let target: any;
    try {
        target = new URL(base);
    }
    ...
    const common: Record<string, any> = { schema_version: "dlr-cmdb-upsert/v1", source_scope: scope, scan_id: scan };
    ...
    try {
        ...
        const failedSummary: Record<string, any> = { assets: acknowledgedAssets, relationships: 0, pages: summary.pages, failures: ["target_batch"] };
...
export async function handle(context: Context, input: any): Promise<any> {
    ...
    if (!Number.isInteger(rawOffset) || rawOffset < 0 || rawOffset > 2147483647)
        throw new Error("invalid_offset");
    let offset = rawOffset;
    if (mode === "sync" && offset !== 0)
        throw new Error("sync_offset_must_be_zero");
    ...
    if (typeof encodedQuery !== "string" || [...encodedQuery].length > 4096)
        throw new Error("invalid_encoded_query");
    let base: any;
    try {
        base = new URL(input.instance_url);
    }
    catch {
    ...
    if ((input.table ?? "cmdb_ci") !== "cmdb_ci")
        throw new Error("only_cmdb_ci_supported");
    const fields = input.fields ?? ["sys_id", "name", "sys_class_name", "install_status"];
    if (!Array.isArray(fields) || fields.length > 64 || !fields.includes("sys_id") || fields.some((field: any) => typeof field !== "string" || !/^[A-Za-z][A-Za-z0-9_.]{0,127}$/.test(field)))
    ...
    const assets = new Map<any, any>(), assetSizes = new Map<any, any>();
    let assetItemBytes = 0, totalBytes = 0, pages = 0, partial = false, failure: any = null;
    ...
    try {
        for (let pageAt = 0; pageAt < maxPages; pageAt += 1) {
            ...
            url.searchParams.set("sysparm_display_value", String(displayValue));
            url.searchParams.set("sysparm_exclude_reference_link", "true");
            url.searchParams.set("sysparm_query", encodedQuery);
            ...
            for (const record of records) {
                ...
                if (candidateCount > maxRecords) {
                    partial = true;
                    break;
                }
                if (!candidateFits(candidateCount, candidateItemBytes, pages, offset + processed + 1, maxBytes)) {
                    partial = true;
                    failure = "max_bytes_exceeded";
                    break;
                }
                assets.set(mapped.external_key, mapped);
                assetSizes.set(mapped.external_key, assetBytes);
            ...
            if (assets.size >= maxRecords) {
                partial = true;
                break;
            }
        }
        if (pages === maxPages && !sourceComplete)
            partial = true;
    }
    catch {
        partial = true;
    ...
    const ordered = [...assets.values()].sort((a: any, b: any) => a.external_key.localeCompare(b.external_key));
    if (mode === "preview") {
    ...
    const summary: Record<string, any> = { assets: ordered.length, relationships: 0, pages, failures: [] };

...
def _column_names(cursor: object, raw_row: object) -> list[str]:
    ...
    if description:
        names = [
            item.name if isinstance(getattr(item, "name", None), str) else str(item[0])
...
def handle(context, input):
    ...
    dsn = context.secrets.get("POSTGRES_DSN")
    if not dsn:
    ...
    try:
        ...
        connection = psycopg.connect(dsn, connect_timeout=min(timeout, 60))
        failure_code = "database_query_failed"
        connection.execute("SET TRANSACTION READ ONLY")
        connection.execute("SET LOCAL TIME ZONE 'UTC'")
        connection.execute(
            "SELECT set_config('statement_timeout', %s, true)", (str(timeout * 1000),)
        )
        with connection.cursor(name="dlr_snapshot") as cursor:

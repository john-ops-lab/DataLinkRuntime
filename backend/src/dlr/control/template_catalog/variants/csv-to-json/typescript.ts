...
function* parseCsv(text: any, delimiter: any, maxColumns: any, maxField: any): Generator<any, void, unknown> {
    let row: any[] = [];
    let field = "";
    let fieldBytes = 0;
    let quoted = false;
    ...
    const append = (value: any) => {
        ...
        if (fieldBytes > maxField)
            throw new Error("field_limit_exceeded");
    };
    const finishField = () => {
        row.push(field);
        if (row.length > maxColumns)
            throw new Error("column_limit_exceeded");
        field = "";
        fieldBytes = 0;
    };
...
function validateHeaders(headers: any, maxColumns: any, maxField: any): any {
    if (headers.length === 0 || new Set<any>(headers).size !== headers.length
        || headers.some((item: any) => item.trim().length === 0)) {
        throw new Error("invalid_or_duplicate_header");
    }
    if (headers.length > maxColumns)
        throw new Error("column_limit_exceeded");
    if (headers.some((item: any) => Buffer.byteLength(item, "utf8") > maxField)) {
        throw new Error("field_limit_exceeded");
    }
}
function run(context: Context, input: any): any {
    ...
    const maxInput = positive(input.max_input_bytes, 2097152, 16777216);
    const maxOutput = positive(input.max_output_bytes, 4194304, 16777216);
    const maxRows = positive(input.max_rows, 10000, 100000);
    const maxColumns = positive(input.max_columns, 200, 2000);
    const maxField = positive(input.max_field_bytes, 65536, 1048576);
    ...
    if (encoding === null)
        throw new Error("invalid_encoding_or_content");
    let bytes: any;
    if (typeof input.content === "string")
        bytes = Buffer.from(input.content, encoding);
    else if (context.inputFiles.length > 0) {
        if (context.inputFiles[0].sizeBytes > maxInput)
    ...
    if (typeof delimiter !== "string" || [...delimiter].length !== 1)
        throw new Error("invalid_delimiter");
    let headers = input.headers;
    if (headers !== undefined && (!Array.isArray(headers) || headers.some((item: any) => typeof item !== "string"))) {
    ...
    for (const parsed of parseCsv(text, delimiter, maxColumns, maxField)) {
        ...
        if (input.skip_empty !== false && row.every((cell: any) => cell.trim().length === 0))
            continue;
        ...
        if (headers && row.length > headers.length)
            throw new Error("row_has_extra_columns");
        const item = headers
            ? Object.fromEntries(headers.map((name: any, index: any) => [name, row[index] ?? null]))
            : row;

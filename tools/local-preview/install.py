...
def main():
    ...
    # Pause before waiting on operations: never replace in-flight controller code.
    ...
    try:
        with preview.operation_lock(blocking=False):
            # Operation then configuration is the global lock order. Holding both
            # prevents plan/resume races while the installed controller is replaced.
            with preview.config_lock():
                if preview.read("config.json") != paused:
                    parser.error(
                        "Controller selection changed after pausing; rerun install"
                    )
                state = preview.read("state.json")
                if not state or not state.get("sha"):
                    parser.error("Deployed state changed after pausing; rerun install")
                if preview.read("attention.json"):
                    parser.error(
                        "Resolve the unfinished deployment before replacing the controller"
                    )
                message = (preview.read("status.json", {}) or {}).get(
                    "message", ""
                ).lower()
                if any(
                    word in message
                    for word in ("deploying", "recovering", "building", "switching")
                ):
                    parser.error(
                ...
                with (root / "controller.lock").open("a") as lock:
                    try:
                        ...
                        parser.error(
                            "Watcher is still running; it remains paused, rerun install"
                        )
                    backup = (
                        root
                        / "controller-backups"
                    ...
                    shutil.copy2(
                        source.parents[1] / "docs/zh-CN/local-preview.md",
                        root / "README.md",
                    )
                    ...
                    launch.setdefault("EnvironmentVariables", {})[
                        "DLR_PREVIEW_HOME"
                    ] = str(root)

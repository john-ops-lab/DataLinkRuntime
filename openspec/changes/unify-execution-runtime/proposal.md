...
不提供旧二进制和旧执行数据的在线迁移兼容。本次测试数据库可重建，但必须先确认其归属及内容；其他数据库、卷和容器不在范围内。历史 Alembic 链保留，用新迁移收敛现行约束。回滚是重新部署对应版本的干净环境，而非将新行交给旧执行器。原始阶段的提交与发布边界已由 Issue #144 的明确授权更新：继续 PR #143，完成限定修复和真实验收后合并 main、发布 v0.4.1；用户视觉验收仍独立报告。

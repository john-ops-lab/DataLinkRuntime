...
- `incident-preserving-upgrade`：冻结无 active Attempt 的特定 Incident 责任，保留原记录完成同机制向前升级。
...
- 部署：`tools/local-preview/` 与双语部署说明；私有地址、配置、备份、真实记录/快照和证据不进入公共仓库。保持固定项目、持久卷、Token、Master Key。

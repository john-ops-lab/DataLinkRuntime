...
- [ ] 1.5 集中定义 InputConfig 与兼容期稳定错误 code/API 响应，验证 schema 快照不包含 Artifact storage key、路径、Token 或文件内容。
- [ ] 1.6 运行 A0 Gate：`uv run --frozen --project backend pytest <A0 migration/input-config tests> -q`、`uv run --frozen --project backend ruff check <changed backend files>`、`ruff format --check`、`mypy` 相关模块；全部 PASS 才形成 A0 Candidate。

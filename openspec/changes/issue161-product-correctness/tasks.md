...
- [ ] 10.3 按用户选择的测试验收路径部署：核精确候选 CI/独立审查和镜像，在原固定 project/双入口使用独立六卷及日志；封存旧卷和历史证据，旧 watcher 暂停且不加载，不执行 D13。私有编排配置与实际挂载核对通过，完成 schema、健康、Worker 隔离、RabbitMQ→Worker 运行及自然 cleanup，保存独立 test-acceptance receipt，不伪造旧 v4 状态。
- [ ] 10.4 在唯一开放非 draft PR 的精确 HEAD 完成实际候选部署、剩余 Chrome/双入口业务验收与独立结果审查后合并；等待精确 merged-main CI，在同一验收卷部署/核对合并版本并保留必要样本。记录各 SHA，禁止先合并后补候选验收，不自动恢复旧 watcher。
...
| 原任务 | 已有工作与当前处理 |
| --- | --- |
| 10.6–10.7 / D11 | 实现、测试及真实失败证据保留；历史失败不改记成功、不重新执行。 |
| 10.8–10.9 / D12 | 已完成的受限收尾及独立验证原件保留，不重做；不再要求由它接续 v4 部署。 |
| 10.10–10.11 / D13 | 恢复代码、155 项控制器测试、独立审查和 d080c37 四项 CI 已完成；结果保留，不代替实际执行证明。 |
| 10.12 / D13 执行 | 根据用户最新测试环境范围取消本轮执行，待批请求搁置；不启动旧容器、不新增恢复批准链。 |

上述路径的严格实现检查不作放宽，历史证据与实际状态仍分别记账。当前剩余工作以 1–9 的功能验收和 10.1–10.5 的测试环境交付为准。

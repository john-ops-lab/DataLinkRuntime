...
用户已选择测试环境验收路径。以下既有保全要求继续约束显式选择对应旧模式的行为，但旧模式的执行不再作为本轮功能交付前置条件；本轮采用下述独立验收基线要求。既有成功、失败及未执行状态保持，不重放旧流程或改写历史证明。
...
### Requirement: 测试验收基线与历史保全链分离
本轮交付 SHALL 使用原固定 Compose project 和双入口、独立验收数据卷及日志，保留原卷、凭据和历史证据；MUST NOT 将旧卷或日志挂入新验收服务、删除旧数据或把新验收结果标记为旧 v4/D13 恢复成功。原自动 watcher MUST 保持暂停且不运行；实际部署事实 SHALL 由独立 test-acceptance receipt 记录。

#### Scenario: 配置渲染意外指向旧存储
- **WHEN** 候选 Compose 配置或实际容器挂载包含旧卷、旧日志目录、非预期端口或不符合原资源隔离要求
- **THEN** 验收编排拒绝继续或报告失败，不启动旧责任处理，也不声明部署通过

#### Scenario: 在新验收样本上完成业务验证
- **WHEN** 精确候选 CI 和独立审查通过，且新验收数据库正常初始化
- **THEN** 通过真实 RabbitMQ 与 Worker 执行、自然 cleanup、双入口健康和认证、Chrome 业务交互验证本组功能；旧生产升级身份连续性不作为门禁，历史结果按明确源码依据继承

#### Scenario: 合并版本继续使用验收基线
- **WHEN** 候选已完成必要验收并获既有合并授权，merged-main CI 成功
- **THEN** 在同一验收卷和入口部署或核对合并版本，保留有用测试样本和真实版本证明；相关主 Issue 保持开放，旧 watcher 不自动恢复

...
用户明确本机环境用于测试验收，不以模拟生产原地升级为目标。本轮改为在原固定 Compose project 和双入口部署精确候选，使用独立命名的验收数据卷及日志目录；原六卷、旧日志、凭据和已有证据封存保留，不挂入新验收服务，不重放历史排队任务。必要的功能样本由已有合成夹具建立，已完成的真实运行结果按产品源码不变的依据继承，不重新执行已完成的事故处置。

下述 D10–D13 保全能力及其历史批准继续作为已有实现和证据记录保留，只有显式选择对应旧路径时适用；它们不再是本轮候选部署、页面验收与合并的前置条件，D13 待批请求搁置且不执行。本轮不宣称生产升级兼容性或旧历史链恢复成功。仍须通过最终 HEAD CI、独立审查、真实候选部署、双入口业务及 Chrome 验收、merged-main CI 和部署核对；相关主 Issue 在用户最终验收前保持开放。

验收编排仅保存在私有交付目录，复用现有 Compose、镜像和运行探针，不新增公开控制器功能。原自动 watcher 保持暂停并卸载驻留项，旧控制面记录保留为历史，不伪造新的 v4 Ready；独立验收 receipt 记录实际运行版本、镜像、存储映射、检查结果和后继运行方式。
...
- 按本轮新增范围批准，为上述受限收尾成功后这一次 VM 重启准备单次旧软件启动和独立保全来源：先从已闭合的原收尾链验证 stopped 现场，按原参数重建唯一 keeper，仅启动原 PostgreSQL/RabbitMQ/Control/Worker/Web 容器，account-web 保持停止；分阶段核完整数据库、文件、日志和本次 boot/startup 证明，成功后只追加可重算来源。代码、测试和独立审查可以推进；实际启动仍须绑定最终工具、请求与动作的专项批准。
...
本次 VM 重启承接仅在 D13 的固定父收尾结果、当前 boot、旧成功软件和原容器绑定内成立，不扩大 D12 的历史证据例外。旧数据库、文件、完整审计及排队记录不得消费或修补；原 state/current/transaction、暂停配置、旧成功与失败原件均保持字节不变。候选镜像的纯构建缓存不算 live compatibility、可信 stage 或部署通过；新增实现继续使用上述六文件及四 planning 路径白名单，不新增 helper、安装器、配置字段或通用 retry/resume。

...
# Only for the separately reviewed post-disposition Web successor:
python3 "$DLR_PREVIEW_HOME/preview.py" plan-carry-forward \
...
For manifest v2, the only same-schema carry-forward supported by this controller is `0040_issue152_dispositions` to that same revision, with no schema objects added or removed. It requires a fresh manifest for the new candidate SHA and only the existing supported responsibility classes qualify. The `execution_incident_dispositions` audit table must exist and remain empty when planning and at every later check; an existing disposition rejects v2 planning before any service is stopped. The existing `runtime_reconciliation_cursors` table remains in the original inventory, and the successor does not rerun the 0039 seed. Its current cursor is not required to equal the initial `0/0`; the normal reconciler may advance it while the application is running. Unknown same-revision or forward transitions are rejected during manifest validation. This explicit v2 successor path is not a general same-schema deployment mechanism.

A separately reviewed exception exists for the fixed post-disposition Web update. It requires `--mode audited-web-same-schema-v1`; omitting the flag keeps the audit-empty manifest v2 contract above. This mode creates manifest v3, is limited to `0040_issue152_dispositions` → `0040_issue152_dispositions`, and recomputes the complete Git object difference during planning, selection, and the final pre-switch check. The only product-source change it accepts is `web/src/index.css`; its test, controller, bilingual documentation, and Issue 161 planning files form a closed companion list. Adds, deletes, renames, symlinks, submodules, mode changes, dependency or migration changes, and unknown paths are rejected.

The private IDs file for this mode adds a non-empty `terminal_executions` list. Each entry binds one original Execution, Incident, disposition UUID, terminal status and generation, output digest, both error codes, and Attempt count. Populate these expectations from the separately sealed and reviewed acceptance snapshot, rather than copying fresh rows into a self-approving plan. Queued and terminal identities cannot overlap; a terminal identity may also appear in `cleanup_execution_ids`, and pending or deferred terminal cleanup must be selected there. Planning reads the complete 17-column audit table and its actual `id` primary key in the same read-only transaction as the other thirteen responsibility tables. It requires the whole audit table to equal the explicit disposition set, checks actor, request digest, Incident and Outbox relationships, terminal result, no replay, released resources, and Adapter/global Admission totals, then preserves the complete fourteen-table projection at every later phase. A previously published cancellation Outbox row remains unchanged, including a null `last_error_code`; the canonical cancellation code belongs to Execution and disposition audit facts.

...
The only same-schema carry-forward supported by this controller is `0040_issue152_dispositions` to that same revision, with no schema objects added or removed. It requires a fresh manifest for the new candidate SHA and only the existing supported responsibility classes qualify. The `execution_incident_dispositions` audit table must exist and remain empty when planning and at every later check; an existing disposition rejects planning before any service is stopped. The existing `runtime_reconciliation_cursors` table remains in the original inventory, and the successor does not rerun the 0039 seed. Its current cursor is not required to equal the initial `0/0`; the normal reconciler may advance it while the application is running. Unknown same-revision or forward transitions are rejected during manifest validation. This explicit successor path is not a general same-schema deployment mechanism.

...
- 可见紧凑文案把数量、单文件大小、总大小和隐私提醒放在同一 flex 行；没有
  `<br>`、可见块级分段或会导致换行的主动规则。
...
- focused render/regression：`assistant-wave-b3.test.tsx` Batch 6 与 `assistant-wave-b.test.tsx` `M5.8-009` 通过。
- auxiliary Chromium：`auxiliary-matrix/browser-report.json` 的 8 个用例全部通过，覆盖
  `zh-CN`/`en` × `1280`/`1440`/`1680`/`1920`；每例断言单行、`flex-wrap`、子元素
  `white-space`、`scrollWidth <= clientWidth`、整页水平/垂直溢出、上传/拖拽/错误/移除、
  lifecycle 和 fixture assist request。
...
- AO Browser primary artifact 采用 sanitized snapshot/request/console/error report；
  ...
  AO screenshot。视觉和 overflow 证据只引用下方辅助 Chromium 的八张截图。
